import React, { useMemo, useState } from 'react';
import { useSearchParams } from 'react-router-dom';
import { AppLayout } from '@/components/AppLayout';
import { useData } from '@/contexts/DataContext';
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from '@/components/ui/select';
import { Button } from '@/components/ui/button';

const HistoryPage: React.FC = () => {
  const { classes, students, attendance } = useData();
  const [searchParams, setSearchParams] = useSearchParams();

  const statusFilter = searchParams.get('status'); // "present" | "absent" | null
  const [filterClass, setFilterClass] = useState('all');
  const [filterDate, setFilterDate] = useState('');

  const records = useMemo(() => {
    return attendance
      .filter((a) => {
        if (filterDate && a.attendance_date !== filterDate) return false;

        if (statusFilter === 'present' && a.status !== 'P') return false;
        if (statusFilter === 'absent' && a.status !== 'A') return false;

        const student = students.find((s) => s.id === a.student_id);
        if (!student) return false;

        if (filterClass !== 'all' && student.class_id !== filterClass) return false;

        return true;
      })
      .map((a) => ({
        ...a,
        student: students.find((s) => s.id === a.student_id)!,
      }))
      .sort((a, b) => {
        if (b.attendance_date !== a.attendance_date) {
          return b.attendance_date.localeCompare(a.attendance_date);
        }
        return a.student.full_name.localeCompare(b.student.full_name);
      });
  }, [attendance, students, filterClass, filterDate, statusFilter]);

  const grouped = useMemo(() => {
    const map = new Map<string, typeof records>();

    records.forEach((r) => {
      const existing = map.get(r.attendance_date) || [];
      existing.push(r);
      map.set(r.attendance_date, existing);
    });

    return Array.from(map.entries()).sort((a, b) => b[0].localeCompare(a[0]));
  }, [records]);

  const clearStatusFilter = () => {
    const next = new URLSearchParams(searchParams);
    next.delete('status');
    setSearchParams(next);
  };

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex flex-col gap-1">
          <h1 className="text-xl font-bold">Attendance History</h1>
          <p className="text-sm text-muted-foreground">
            Review past attendance records by class, date, and status.
          </p>
        </div>

        {statusFilter && (
          <div className="flex items-center justify-between rounded-xl border border-primary/20 bg-primary/5 px-4 py-3">
            <p className="text-sm">
              Active filter:{' '}
              <span className="font-semibold text-primary capitalize">
                {statusFilter}
              </span>
            </p>
            <Button variant="ghost" size="sm" onClick={clearStatusFilter}>
              Clear
            </Button>
          </div>
        )}

        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <Select value={filterClass} onValueChange={setFilterClass}>
            <SelectTrigger className="h-10">
              <SelectValue placeholder="Select class" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All Classes</SelectItem>
              {classes.map((c) => (
                <SelectItem key={c.id} value={c.id}>
                  {c.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>

          <input
            type="date"
            value={filterDate}
            onChange={(e) => setFilterDate(e.target.value)}
            className="h-10 rounded-lg border border-input bg-background px-3 text-sm"
          />
        </div>

        <div className="space-y-4">
          {grouped.map(([date, recs]) => {
            const present = recs.filter((r) => r.status === 'P').length;
            const absent = recs.filter((r) => r.status === 'A').length;

            return (
              <div key={date} className="rounded-xl border border-border bg-card">
                <div className="flex items-center justify-between border-b border-border p-4">
                  <span className="text-sm font-semibold">
                    {new Date(`${date}T00:00:00`).toLocaleDateString('en-IN', {
                      weekday: 'short',
                      day: 'numeric',
                      month: 'short',
                      year: 'numeric',
                    })}
                  </span>

                  <div className="flex gap-3 text-xs">
                    <span className="font-medium text-success">{present}P</span>
                    <span className="font-medium text-destructive">{absent}A</span>
                  </div>
                </div>

                <div className="divide-y divide-border">
                  {recs.map((r) => (
                    <div
                      key={r.id}
                      className="flex items-center justify-between px-4 py-2.5"
                    >
                      <div>
                        <span className="text-sm font-medium">
                          {r.student.full_name}
                        </span>
                        <span className="ml-2 text-xs text-muted-foreground">
                          #{r.student.roll_number}
                        </span>
                      </div>

                      <span
                        className={`rounded px-2 py-1 text-xs font-bold ${
                          r.status === 'P'
                            ? 'status-present-light'
                            : 'status-absent-light'
                        }`}
                      >
                        {r.status === 'P' ? 'Present' : 'Absent'}
                      </span>
                    </div>
                  ))}
                </div>
              </div>
            );
          })}

          {grouped.length === 0 && (
            <p className="py-8 text-center text-muted-foreground">
              No attendance records found
            </p>
          )}
        </div>
      </div>
    </AppLayout>
  );
};

export default HistoryPage;